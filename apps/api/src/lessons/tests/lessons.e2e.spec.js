...
describe("Lessons (e2e)", () => {
    ...
    describe("CRUD", () => {
        ...
        it("POST /lessons should create a lesson", async () => {
            ...
            expect(res.body.weekOf.startsWith(base.weekOfISO)).toBe(true);
        ...
        it("GET /lessons should filter by tenantId/groupId/weekOf", async () => {
            ...
            expect(res.body.every((l) => l.tenantId === ids.tenant &&
                l.groupId === ids.group &&
                typeof l.weekOf === "string" &&
                l.weekOf.startsWith(base.weekOfISO))).toBe(true);
        });
        it("GET /lessons should not leak other tenant lessons", async () => {
            const res = await request(app.getHttpServer())
                ...
                .set("Authorization", authHeader);
            expect(res.status).toBe(200);
            expect(res.body.every((l) => l.tenantId === ids.tenant)).toBe(true);
            expect(res.body.some((l) => l.id === otherLessonId)).toBe(false);
        });
        it("PATCH /lessons/:id should update fields", async () => {
            ...
            expect(typeof res.body.weekOf).toBe("string");
            expect(res.body.weekOf.startsWith("2025-01-13")).toBe(true);
        });
