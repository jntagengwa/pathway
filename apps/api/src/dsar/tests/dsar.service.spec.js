...
describe("DsarService", () => {
    ...
    it("exports child data with related entities", async () => {
        ...
        expect(prismaMock.child.findFirst).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("throws NotFound when child is not in tenant", async () => {
        ...
        await expect(service.exportChild(childId, tenantId)).rejects.toBeInstanceOf(NotFoundException);
    });
});
