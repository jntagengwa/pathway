...
jest.mock("@pathway/db", () => {
    ...
    return {
        prisma: tx,
        withTenantRlsContext: (_tenantId, _orgId, cb) => cb(tx),
    };
});
describe("ConcernsService", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        service = new ConcernsService(auditStub);
    });
    describe("create", () => {
        ...
        it("throws NotFound when child missing", async () => {
            ...
            await expect(service.create({ childId, summary: "x" }, context)).rejects.toBeInstanceOf(NotFoundException);
        ...
        it("maps Prisma FK error to BadRequest", async () => {
            ...
            await expect(service.create({ childId, summary: "x" }, context)).rejects.toBeInstanceOf(BadRequestException);
        });
    });
    describe("findAll", () => {
    ...
    describe("findOne", () => {
        ...
        it("throws NotFound when missing", async () => {
            ...
            await expect(service.findOne(concernId, context)).rejects.toBeInstanceOf(NotFoundException);
        });
    });
    describe("update", () => {
        ...
        it("maps P2025 to NotFound on update", async () => {
            ...
            await expect(service.update(concernId, { summary: "x" }, context)).rejects.toBeInstanceOf(NotFoundException);
        });
    });
    describe("remove", () => {
        ...
        it("maps P2025 to NotFound on delete", async () => {
            ...
            concernUpdate.mockRejectedValue({ code: "P2025", message: "not found" });
            await expect(service.remove(concernId, context)).rejects.toBeInstanceOf(NotFoundException);
        });
        it("maps P2003 to BadRequest on delete", async () => {
            ...
            await expect(service.remove(concernId, context)).rejects.toBeInstanceOf(BadRequestException);
        });
    });
});
