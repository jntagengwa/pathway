...
import { withTenantRlsContext, AssignmentStatus, Role, SwapStatus, } from "@pathway/db";
...
describe("Swaps (e2e)", () => {
    ...
    it("GET /swaps should filter by fromUserId and status", async () => {
        ...
        expect(res.body.every((s) => s.fromUserId === ids.userFrom && s.status === SwapStatus.REQUESTED)).toBe(true);
    });
