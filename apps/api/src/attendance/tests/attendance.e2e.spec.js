...
if (!TENANT_A_ID || !TENANT_B_ID || !ORG_ID) {
    throw new Error("E2E_TENANT_ID / E2E_TENANT2_ID / E2E_ORG_ID are missing. Ensure test.setup.e2e.ts seeds tenants and exports their IDs.");
}
describe("Attendance (e2e)", () => {
    ...
    it("GET /attendance should not leak other tenant records", async () => {
        const childB = await withTenantRlsContext(TENANT_B_ID, ORG_ID, async (tx) => tx.child.create({
            data: {
                firstName: "Other",
                lastName: "Child",
                tenantId: TENANT_B_ID,
        ...
        await withTenantRlsContext(TENANT_B_ID, ORG_ID, async (tx) => tx.attendance.create({
            data: {
                childId: childB.id,
                groupId: ids.group2,
                present: true,
        ...
        const res = await request(app.getHttpServer())
            ...
            .set("Authorization", authHeader);
        expect(res.status).toBe(200);
        expect(res.body.every((row) => row.childId !== childB.id)).toBe(true);
    });
    it("GET /attendance/:id should 404 for other tenant record", async () => {
        ...
        const attendanceB = await withTenantRlsContext(TENANT_B_ID, ORG_ID, async (tx) => tx.attendance.create({
            data: {
                childId: childB.id,
                groupId: ids.group2,
                present: false,
