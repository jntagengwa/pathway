...
describe("Groups (e2e)", () => {
    ...
    beforeAll(async () => {
        ...
        const seededTenant = process.env.E2E_TENANT_ID;
        const seededTenant2 = process.env.E2E_TENANT2_ID;
    ...
    it("GET /groups should return array", async () => {
        ...
        expect(res.body.every((g) => g.tenantId === tenantId)).toBe(true);
    });
    ...
    it("GET /groups/:id should 404 when accessing other tenant", async () => {
        if (!otherGroupId)
            throw new Error("otherGroupId missing");
        const res = await request(app.getHttpServer())
