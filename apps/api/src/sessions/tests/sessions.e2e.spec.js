...
describe("Sessions (e2e)", () => {
    ...
    const tenantId = process.env.E2E_TENANT_ID; // main tenant
    const otherTenantId = process.env.E2E_TENANT2_ID; // other tenant
    ...
    it("PATCH /sessions/:id should update a session", async () => {
        ...
        expect(res.status).toBe(200);
        expect(new Date(res.body.endsAt).toISOString()).toBe(new Date("2025-01-01T17:00:00.000Z").toISOString());
    });
    ...
    it("GET /sessions should not leak other tenant records", async () => {
        ...
        expect(res.status).toBe(200);
        expect(Array.isArray(res.body)).toBe(true);
        expect(res.body.every((s) => s.tenantId === otherTenantId)).toBe(true);
    });
