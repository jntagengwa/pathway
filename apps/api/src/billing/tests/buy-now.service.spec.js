...
describe("BuyNowService", () => {
    ...
    it("computes preview caps for known plan and calls provider", async () => {
        const service = new BuyNowService(previewService, providerMock, contextMock, providerConfig);
        const result = await service.checkout({
            ...baseRequest,
            plan: { planCode: "STARTER_MONTHLY", av30AddonBlocks: 2 },
        });
        expect(providerMock.createCheckoutSession).toHaveBeenCalledTimes(1);
        expect(providerMock.createCheckoutSession).toHaveBeenCalledWith(expect.objectContaining({ pendingOrderId: "po_1" }), expect.objectContaining({ orgId: "org_1", tenantId: "tenant_1" }));
        expect(prismaMock.pendingOrder.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("handles unknown plan by using add-ons only and warns", async () => {
        ...
        expect(result.warnings).toEqual(expect.arrayContaining(["price_not_included", "unknown_plan_code"]));
    });
