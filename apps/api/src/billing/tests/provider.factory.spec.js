import { createBuyNowProvider, createBillingWebhookProvider, } from "../providers/provider.factory";
...
describe("provider factory", () => {
    it("creates Stripe providers when active is STRIPE", () => {
        const config = { ...baseConfig, activeProvider: "STRIPE" };
        expect(createBuyNowProvider(config)).toBeInstanceOf(StripeBuyNowProvider);
        expect(createBillingWebhookProvider(config)).toBeInstanceOf(StripeBillingWebhookProvider);
    });
    it("creates GoCardless providers when active is GOCARDLESS", () => {
        const config = { ...baseConfig, activeProvider: "GOCARDLESS" };
        expect(createBuyNowProvider(config)).toBeInstanceOf(FakeBuyNowProvider);
        expect(createBillingWebhookProvider(config)).toBeInstanceOf(FakeBillingWebhookProvider);
    });
    it("creates fake providers by default", () => {
        const config = { ...baseConfig, activeProvider: "FAKE" };
        expect(createBuyNowProvider(config)).toBeInstanceOf(FakeBuyNowProvider);
        expect(createBillingWebhookProvider(config)).toBeInstanceOf(FakeBillingWebhookProvider);
    });
});
