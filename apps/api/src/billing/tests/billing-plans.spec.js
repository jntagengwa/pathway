import { PLAN_CATALOGUE, getPlanDefinition, } from "../billing-plans";
describe("billing-plans catalogue", () => {
    ...
    it("returns definitions for each known plan code", () => {
        allPlanCodes.forEach((code) => {
            ...
            expect(definition?.tier).toBe(code.startsWith("STARTER")
                ? "starter"
                : code.startsWith("GROWTH")
                    ? "growth"
                    : "enterprise");
            expect(definition?.selfServe).toBe(code === "ENTERPRISE_CONTACT" ? false : true);
        });
    });
