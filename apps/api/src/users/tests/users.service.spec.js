...
const uFindMany = prisma.user.findMany;
const uFindUnique = prisma.user.findUnique;
const uFindFirst = prisma.user.findFirst;
const uCreate = prisma.user.create;
const uUpdate = prisma.user.update;
const cFindMany = prisma.child.findMany;
describe("UsersService", () => {
    ...
    describe("list", () => {
        it("returns users", async () => {
            ...
            expect(uFindMany).toHaveBeenCalledWith(expect.objectContaining({ where: { tenantId: "t1" } }));
        });
    });
    describe("getById", () => {
        ...
        it("throws NotFound when missing", async () => {
            ...
            await expect(svc.getById("missing", "t1")).rejects.toBeInstanceOf(NotFoundException);
        });
    });
    describe("create", () => {
        it("normalizes email and creates user (with tenant connect)", async () => {
            ...
            expect(uCreate).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("update", () => {
        it("updates user fields and returns selection", async () => {
            ...
            expect(uUpdate).toHaveBeenCalledWith(expect.objectContaining({
                where: { id: "u1" },
        ...
        it("400 when updating children with missing ids", async () => {
            uFindUnique.mockResolvedValue({ tenantId: "t1" });
            cFindMany.mockResolvedValue([{ id: "c1", tenantId: "t1" }]); // c2 missing
            await expect(svc.update("u1", { children: ["c1", "c2"] })).rejects.toBeInstanceOf(BadRequestException);
            expect(uUpdate).not.toHaveBeenCalled();
        });
        it("400 when updating children cross-tenant (target tenant is current)", async () => {
            uFindUnique.mockResolvedValue({ tenantId: "t1" });
            cFindMany.mockResolvedValue([{ id: "c9", tenantId: "t9" }]);
            await expect(svc.update("u1", { children: ["c9"] })).rejects.toBeInstanceOf(BadRequestException);
            expect(uUpdate).not.toHaveBeenCalled();
        });
        it("throws BadRequest on unique email conflict (P2002)", async () => {
            const p2002 = Object.assign(new Error("Unique"), { code: "P2002" });
            uUpdate.mockRejectedValue(p2002);
            await expect(svc.update("u1", { email: "dup@example.com" })).rejects.toBeInstanceOf(BadRequestException);
        });
