...
describe("Users (e2e)", () => {
    ...
    beforeAll(async () => {
        ...
        const otherUser = await withTenantRlsContext(otherTenantId, orgId, async (tx) => tx.user.create({
            data: {
    ...
    it("GET /users should return array", async () => {
        ...
        expect(res.body.every((u) => u.tenantId === tenantId)).toBe(true);
    });
    ...
    it("GET /users/:id should return the created user", async () => {
        ...
        const created = await withTenantRlsContext(tenantId, orgId, (tx) => tx.user.findFirst({ where: { email } }));
    ...
    it("GET /users should not leak other tenants", async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.body.every((u) => u.tenantId === tenantId)).toBe(true);
        expect(res.body.some((u) => u.id === otherTenantUserId)).toBe(false);
    });
