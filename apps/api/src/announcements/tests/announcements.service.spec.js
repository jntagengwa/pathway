...
const announcementUpdate = jest.fn();
const announcementDelete = jest.fn();
const announcementFindFirst = jest.fn();
const announcementFindMany = jest.fn();
jest.mock("@pathway/db", () => ({
    prisma: {
        tenant: { findUnique: (arg) => tenantFindUnique(arg) },
        announcement: {
            create: (arg) => announcementCreate(arg),
            update: (arg) => announcementUpdate(arg),
            delete: (arg) => announcementDelete(arg),
            findFirst: (arg) => announcementFindFirst(arg),
            findMany: (arg) => announcementFindMany(arg),
        },
    },
}));
describe("AnnouncementsService", () => {
    ...
    describe("create", () => {
        ...
        it("throws NotFound when tenant missing", async () => {
            ...
            await expect(service.create({ tenantId, title: "t", body: "b", audience: "ALL" }, tenantId)).rejects.toBeInstanceOf(NotFoundException);
        ...
        it("maps Prisma FK error to BadRequest", async () => {
            ...
            await expect(service.create({ tenantId, title: "t", body: "b", audience: "ALL" }, tenantId)).rejects.toBeInstanceOf(BadRequestException);
        });
    });
    describe("findAll", () => {
    ...
    describe("findOne", () => {
        ...
        it("throws NotFound when missing", async () => {
            ...
            await expect(service.findOne(id, tenantId)).rejects.toBeInstanceOf(NotFoundException);
        });
    });
    describe("update", () => {
        ...
        it("maps P2025 to NotFound", async () => {
            ...
            await expect(service.update(id, { title: "Updated" }, tenantId)).rejects.toBeInstanceOf(NotFoundException);
        });
    });
    describe("remove", () => {
        ...
        it("maps P2025 to NotFound on delete", async () => {
            ...
            await expect(service.remove(id, tenantId)).rejects.toBeInstanceOf(NotFoundException);
        });
        it("maps P2003 to BadRequest on delete", async () => {
            announcementFindFirst.mockResolvedValue(baseAnnouncement);
            announcementDelete.mockRejectedValue({ code: "P2003", message: "fk" });
            await expect(service.remove(id, tenantId)).rejects.toBeInstanceOf(BadRequestException);
        });
    });
});
