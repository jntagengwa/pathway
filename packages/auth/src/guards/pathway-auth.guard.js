...
let PathwayAuthGuard = PathwayAuthGuard_1 = class PathwayAuthGuard {
    requestContext;
    logger = new Logger(PathwayAuthGuard_1.name);
    ...
    extractClaims(request) {
        ...
        try {
            ...
            this.logger.error("Unable to parse token payload", error instanceof Error ? error.stack : undefined);
            const fallback = this.tryDevTokenFallback();
            if (fallback)
                return fallback;
    ...
    tryDevTokenFallback() {
        if (process.env.NODE_ENV === "production")
            return null;
        const devToken = process.env.NEXT_PUBLIC_DEV_BEARER_TOKEN ?? process.env.DEV_BEARER_TOKEN;
        if (devToken) {
            ...
            if (devPayload) {
                try {
                    ...
                    this.logger.warn("Dev token fallback failed to parse payload", error instanceof Error ? error.message : String(error));
                }
            }
        }
        return DEFAULT_DEBUG_CLAIMS;
    }
    async mapClaimsToContext(claims, request) {
        ...
        const roles = await this.resolveRoles(user.id, tenantCtx.orgId, tenantCtx.tenantId, claims);
        ...
        return {
            ...
            roles,
            permissions: this.parsePermissions(claims["https://pathway.app/permissions"]),
            rawClaims: claims,
            issuedAt: claims.iat,
    ...
    async resolveUserFromClaims(claims) {
        ...
        const name = userClaims.givenName ??
            claims.name ??
    ...
    async resolveTenantContext(userId, claims, request) {
        const userClaims = claims["https://pathway.app/user"] ?? {};
        const orgClaims = claims["https://pathway.app/org"] ?? {};
        ...
        const tenantId = tenantClaims?.tenantId ??
            cookieTenantId ??
        ...
        const orgId = orgClaims.orgId ?? tenantClaims?.orgId ?? cookieOrgId ?? tenant.orgId;
        const siteMembership = await prisma.siteMembership.findFirst({
            where: { tenantId: tenant.id, userId },
        });
        if (!siteMembership) {
            throw new UnauthorizedException("User is not a member of the active site");
        }
        await prisma.user.update({
            where: { id: userId },
    ...
    parseOrgRoles(claims) {
        if (!claims)
            return [];
        return claims
            .filter((value) => ORG_ROLE_VALUES.has(value))
            .map((value) => value);
    }
    parseTenantRoles(claims) {
        if (!claims)
            return [];
        return claims
            .filter((value) => TENANT_ROLE_VALUES.has(value))
            .map((value) => value);
    }
    parsePermissions(claims) {
        if (!Array.isArray(claims)) {
            return [];
        }
        return claims.filter((value) => typeof value === "string" && value.length > 0);
    }
    ...
    async resolveRoles(userId, orgId, tenantId, claims) {
        const siteMembership = await prisma.siteMembership.findFirst({
        ...
        const tenantRolesFromMembership = [];
        ...
        const orgRoles = Array.from(new Set([
...
PathwayAuthGuard = PathwayAuthGuard_1 = __decorate([
    Injectable({ scope: Scope.REQUEST }),
    __metadata("design:paramtypes", [PathwayRequestContext])
], PathwayAuthGuard);
export { PathwayAuthGuard };
